...
def read_labels():
    path = "C:/Users/Bugra/Desktop/masters-thesis/data/liver_labels"
    labels = ["l1", "l2", "l3", "r1", "r2", "r3"]

    label_images = [ nib.load(os.path.join(path, label + ".nii")).get_fdata()[..., 0, :] for label in labels ]
    label_images = [ np.transpose(label, (2, 1, 0)) for label in label_images ]
    label_images = np.concatenate(label_images, axis=0)
...
def prepare_dataset(confidence_map_path=None):
    ...
    image = read_image("C:/Users/Bugra/Desktop/masters-thesis/final_data/original/liver/images.npy")
