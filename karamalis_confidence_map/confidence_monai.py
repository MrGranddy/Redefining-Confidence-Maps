...
class UltrasoundConfidenceMap:
    ...
    Args:
        ...
        gamma (float, optional): Gamma parameter. Defaults to 0.03.
    ...
    def __init__(
        ...
        solve_mode: Literal["scipy", "octave", "cg"] = "scipy",
    ):
    ...
    def get_seed_and_labels(self, data : np.ndarray, sink_mode: str = "all", sink_mask: Optional[np.ndarray] = None) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if sink_mode == "all":
            ...
            # Get projection of the mask to the last row
            # 1. Get the unique columns
    ...
    def _solve_linear_system(self, D, rhs, tol=1.e-6, mode='scipy'):
        ...
        if mode == 'scipy':
            ...
            raise ValueError(f"Unknown solver mode (backend): {mode}")

        return X

    def confidence_estimation(self, A, seeds, labels, beta, gamma, solve_mode):
